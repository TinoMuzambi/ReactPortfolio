...
| `CONTACT_EMAIL_TO` | `pages/api/email/index.ts` | Optional delivery recipient and direct-email fallback returned during a 503 response |
...
`html-react-parser` is intentionally listed in `transpilePackages`. Its dependency graph includes ESM-only modules that must remain bundled for Pages Router ISR in Vercel's Node.js runtime.
